...
import { MeterInfo } from '../services/storage'
...
export default function MeterModal({ open, initial, onClose, onSave }: Props){
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="glass-card max-w-xl w-full p-6 z-10 text-white">
        ...
        <div className="grid grid-cols-1 gap-3">
          <label className="text-sm text-white">Contador
            <input className="w-full bg-white/5 text-white placeholder-gray-400 border border-white/10 rounded px-2 py-2 mt-1" placeholder="Contador" value={form.contador} onChange={e=>update('contador', e.target.value)} />
          </label>
          <label className="text-sm text-white">Correlativo
            <input className="w-full bg-white/5 text-white placeholder-gray-400 border border-white/10 rounded px-2 py-2 mt-1" placeholder="Correlativo" value={form.correlativo} onChange={e=>update('correlativo', e.target.value)} />
          </label>
          ...
          <label className="text-sm text-white">Tipo servicio
            <input className="w-full bg-white/5 text-white placeholder-gray-400 border border-white/10 rounded px-2 py-2 mt-1" placeholder="Tipo servicio" value={form.tipo_servicio} onChange={e=>update('tipo_servicio', e.target.value)} />
          </label>
