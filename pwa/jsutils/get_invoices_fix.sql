-- Patch: Replace get_invoices function to prefer tariff segment matching meter.tipo_servicio
-- Usage: psql -d <db> -f get_invoices_fix.sql
...
DECLARE
  ...
  inv JSONB;
BEGIN
  -- Get meter information including distribuidora (company) and tipo_servicio
  ...
  -- Prefer tariffs that match both company and the meter's tipo_servicio (segment)
  CREATE TEMP TABLE temp_tariffs AS
  SELECT * FROM tariffs
  WHERE deleted_at IS NULL
  AND company = meter_rec.distribuidora
  AND segment = meter_rec.tipo_servicio;

  -- If no tariffs were found matching the meter's segment, fall back to company-level tariffs
  IF NOT EXISTS (SELECT 1 FROM temp_tariffs) THEN
